...
@app.on_message(filters.command("قائمة الانتظار", [".", ""]) & filters.group & ~BANNED_USERS
)
@language
async def ping_com(client, message: Message, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "**✧ المدة :** Unknown Duration Stream\n\nClick on button below to get whole queued list."
        if DUR == "Unknown"
        else "\n- **اضغط زر قائمة الانتظار لمعرفة المسارات في الانتظار**"
    )
    cap = f"""**𝐂𝐑𝐘𝐒𝐓𝐀𝐋 𝐏𝐋𝐀𝐘𝐄 ♪**
...
async def queued_tracks(client, CallbackQuery: CallbackQuery, _):
    ...
    med = InputMediaPhoto(
        media="https://telegra.ph/file/0b17fcae7555d15458fea.jpg",
        caption=_["queue_1"],
    )
    ...
    for x in got:
        j += 1
        if j == 1:
            msg += f'**- قيد التشغيل حاليا** :\n\n✧ العنوان : {x["title"]}\n✧ المدة : {x["dur"]}\n✧ بواسطة : {x["by"]}\n\n'
        elif j == 2:
            msg += f'-** في قائمة الانتظار :\n\n✧ العنوان : {x["title"]}\n✧ المدة : {x["dur"]}\n✧ بواسطة : {x["by"]}\n\n'
        else:
