...
@app.on_message(filters.command(["ميرتي"]) & filters.group & SUDOERS)
@language
...
@app.on_message(filters.command("لوحة الاحصائيات", [".", ""]) & filters.group)
@languageCB
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"اكثر اغنية تم تشغيلها بكرستال\n\n**العنوان :** {title}\n\nتم تشغيلها** {co}0 **مرة"
...
@app.on_message(filters.command("احصائيات كرستال العالمية", [".", ""]) & filters.group)
@languageCB
async def gstats_global(client, message: Message, _):
    mystic = await message.reply_text(_["gstats_1"])
    stats = await get_global_tops()
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"- [مقاطع صوتية](https://t.me/nkqbot) ** تم تشغيلها {count}0 مرة**\n\n"
                else:
                    msg += f"- {title}** تم تشغيلها {count}0 مرة**\n\n"
    ...
    if what in ["Users", "Chats"]:
        for items, count in list_arranged.items():
            ...
            try:
                ...
                continue
            limit += 1
            msg += f"-`{extract}` تم تشغيلها {count}0 مرة في البوت.\n\n"
        temp = (
