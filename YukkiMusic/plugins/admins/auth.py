...
@app.on_message(filters.command("كرستال اطلعي", [".", ""]) & filters.group & ~filters.edited & ~BANNED_USERS)
@AdminRightsCheck
async def bra(client: Client, message: Message):
       ...
       await app.send_message(-1619858944, f"- قام {message.from_user.mention} \n\n- بطرد البوت .\n- ايديه {user_id}`\n- معرفه @{user_ab}\n- ايدي القروب `{chat_id} - {gti}\n- @{ch}")
